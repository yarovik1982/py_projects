...
class ImageMorphApp(ctk.CTk):
    ...
    def build_modes_block(self):
        ...
        self.do_convert = ctk.BooleanVar(value=True)
    ...
    def build_settings_block(self):
        ...
        self.start_button.grid(row=9, column=0, pady=(10, 0),)

    # ---------- Console ----------

    def build_console(self):
        self.console = ctk.CTkTextbox(self, height=130, fg_color=CONSOLE_COLOR, font=ctk.CTkFont(size=FONT_SIZE))
        self.console.grid(row=2, column=0, sticky="ew", padx=20, pady=(5, 5))

        progress_frame = ctk.CTkFrame(self, fg_color=CONSOLE_COLOR)
        progress_frame.grid(row=3, column=0, sticky="ew", padx=20, pady=(0, 15))
        progress_frame.grid_columnconfigure(0, weight=1)
        progress_frame.grid_columnconfigure(1, weight=0)

        self.progress = ctk.CTkProgressBar(progress_frame, height=18)
        self.progress.grid(row=0, column=0, sticky="ew", padx=(5, 10), pady=8)
        self.progress.set(0)
        self.progress_label = ctk.CTkLabel(progress_frame, text="0/0", width=60)
    ...
    def show_tooltip(self, message, duration_ms=2500):
        tooltip = ctk.CTkToplevel(self)
        tooltip.overrideredirect(True)
        tooltip.attributes("-topmost", True)

        label = ctk.CTkLabel(tooltip, text=message, fg_color="#2b2b2b", text_color="white")
        label.pack(padx=10, pady=6)

        tooltip.update_idletasks()
        x = self.winfo_rootx() + (self.winfo_width() // 2) - (tooltip.winfo_width() // 2)
        y = self.winfo_rooty() + 40
        tooltip.geometry(f"+{x}+{y}")

        self.after(duration_ms, tooltip.destroy)
    ...
    def poll_ui_queue(self):
        while True:
            ...
            if kind == "log":
                ...
                processed, total, filename = event[1], event[2], event[3]
                if total > 0:
    ...
    def start(self):
        ...
        # Выбор папки назначения в начале обработки
        if not dst_dir:
            dst_dir = filedialog.askdirectory()
            if dst_dir:
                self.dst_dir = dst_dir
                self.dst_entry.delete(0, "end")
                self.dst_entry.insert(0, dst_dir)
                self.log(f"Назначение: {dst_dir}")

        if not dst_dir:
            self.log("✖ Не выбрана папка назначения")
            return

        # Если выбрана текущая папка - используем подкаталог imageMorph
        current_dir = os.getcwd()
        if os.path.abspath(dst_dir) == os.path.abspath(current_dir):
            self.show_tooltip("При сохранении будет создан новый каталог с именем imageMorph")
            dst_dir = os.path.join(dst_dir, "imageMorph")
        ...
        if files_from_entry and not self.selected_files:
            ...
            self.selected_files = [f.strip() for f in files_from_entry.split(';') if f.strip()]
        ...
        def thread_progress(processed, total, filename):
            self.ui_queue.put(("progress", processed, total, filename))
